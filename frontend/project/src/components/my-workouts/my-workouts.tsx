...
import {
  getCoachWorkouts,
  getCoachWorkoutsLoadingStatus,
} from '../../store/workout-process/workout-selectors';
import {
  DEFAULT_CALORIES_STEP,
  DEFAULT_PRICE_NUMBER,
  DEFAULT_RATING_STEP,
  DEFAULT_WORKOUTS_CATALOG_NUMBER,
  MAX_CALORIES_NUMBER,
  MAX_RATING_NUMBER,
  MIN_CALORIES_NUMBER,
  MIN_RATING_NUMBER,
...
function MyWorkoutsPage() {
  ...
  const [query, setQuery] = useState<WorkoutListQuery>({
    limit: DEFAULT_WORKOUTS_CATALOG_NUMBER,
    page: 1,
  });

  const [formValue, setValue] = useState({
    minPrice: DEFAULT_PRICE_NUMBER,
    maxPrice: workoutsPrices
      ? Math.max.apply(null, workoutsPrices)
      : DEFAULT_PRICE_NUMBER,
  ...
  const filterChangeHandle = (
    ...
    if (
      (name === 'maxCalories' &&
        Number(valueNum) === 0 &&
        formValue.minCalories === 0) ||
      (name === 'minCalories' &&
        Number(valueNum) === 0 &&
        formValue.maxCalories === 0)
    ) {
    ...
    if (name === 'duration') {
      const isChecked = !!(
        query &&
        query?.times &&
        query?.times.find((el) => el === (value as WorkoutTime))
      );
      if (isChecked && query.times) {
        const currentArr = query.times.filter((el) => el !== value);
        evt.target.removeAttribute('checked');
        currentArr.length === 0
          ? setQuery({ ...query, times: undefined })
          : setQuery({ ...query, times: currentArr });
      } else {
                      ...
                      <div className="filter-range">
                        <MultiRangeSlider
                          ...
                          onInput={(e: ChangeResult) => {
                            ...
                            setQuery({
                              ...query,
                              priceMin: Number(e.minValue),
                              priceMax: Number(e.maxValue),
                            });
                          }}
                        />
                      </div>
                      ...
                      <div className="filter-range">
                        <MultiRangeSlider
                          ...
                          onChange={(e: ChangeResult) => {
                            setValue({
                              ...formValue,
                              minCalories: e.minValue,
                              maxCalories: e.maxValue,
                            });
                            setQuery({
                              ...query,
                              caloriesMin: Number(e.minValue),
                              caloriesMax: Number(e.maxValue),
                            });
                          }}
                        />
                      </div>
                      ...
                      <div className="filter-raiting">
                        <MultiRangeSlider
                          ...
                          onChange={(e: ChangeResult) => {
                            setValue({
                              ...formValue,
                              minRating: e.minValue,
                              maxRating: e.maxValue,
                            });
                            setQuery({
                              ...query,
                              ratingMin: Number(e.minValue),
                              ratingMax: Number(e.maxValue),
                            });
                          }}
                        />
                      </div>
