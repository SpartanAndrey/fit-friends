...
const UserInfo = ({ user }: Props): JSX.Element => {
  ...
  const [isReady, setIsReady] = useState(user.isReadyToTrain);
  ...
  const [choosenWorkoutTypes, setChoosenWorkoutTypes] = useState<WorkoutType[]>(user.workoutType);

  const [isNotWorkoutType, setIsNotWorkoutType] = useState(!(user.workoutType.length >= 1 && user.workoutType.length <= 3));
  ...
  const [currentLevel, setcurrentLevel] = useState<UserLevel>(user.level);
  ...
  const [currentLocation, setLocation] = useState(user.location);
    ...
    <section className="user-info-edit">
      ...
      >
        ...
        <div className="user-info-edit__section">
          ...
          {isNotWorkoutType && isEdit && (
            <span className="custom-input__error">
              Необходимо выбрать 1-3 значений
            </span>
          )}
        </div>
        <div
          className={`${
            ...
            isOpened.location ? 'is-open' : 'custom-select--not-selected'
          } user-info${isEdit ? '-edit' : ''}__select`}
        >
          <span className="custom-select__label">Локация</span>
          <div className="custom-select__placeholder">
            ст. м. {currentLocation}
          </div>
          <button
        ...
        <div
          className={`${
            ...
            isOpened.gender ? 'is-open' : 'custom-select--not-selected'
          } user-info${isEdit ? '-edit' : ''}__select`}
        >
        ...
        <div
          className={`${
            ...
            isOpened.level ? 'is-open' : 'custom-select--not-selected'
          } user-info${isEdit ? '-edit' : ''}__select`}
        >
